 ...
 */
package com.swg.jface.TableViewer;
...
public class Team {
  ...
  public boolean led(Player player, int column) {
    ...
    // Go through all the players on the team, comparing the specified player's
    ...
    for (int i = 0, n = players.size(); i < n && led; i++) {
      ...
      case PlayerConst.COLUMN_POINTS:
        if (player.getPoints() < test.getPoints()) led = false;
        break;
      case PlayerConst.COLUMN_REBOUNDS:
        if (player.getRebounds() < test.getRebounds()) led = false;
        break;
      case PlayerConst.COLUMN_ASSISTS:
        if (player.getAssists() < test.getAssists()) led = false;
        break;
      }
    }
    return led;
